...
const ApplicantDetailsPage = () => {
  ...
  const fetchNotes = async () => {
    try {
      const response = await fetch(`http://localhost:3002/proxy/notes/${applicantId}`);
      if (!response.ok) {
    ...
    const fetchData = async () => {
      ...
      try {
        const results = await Promise.allSettled([
          fetch(`http://localhost:3002/proxy/applicant/${applicantId}`),
          fetch(`http://localhost:3002/proxy/education/${applicantId}`),
          fetch(`http://localhost:3002/proxy/work-experience/${applicantId}`),
          fetch(`http://localhost:3002/proxy/skills/${applicantId}`),
          fetch(`http://localhost:3002/proxy/notes/${applicantId}`)
        ]);
  ...
  if (newContent) {
    try {
      const response = await fetch(`http://localhost:3002/proxy/notes/${noteId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
